...
// Added by Anand Kumar 2023
Beam3dThermalAction::Beam3dThermalAction(int tag,
	double locY1, double locY2, double locZ1, TimeSeries* theSeries, double locZ2,
	int theElementTag)
	:ElementalLoad(tag, LOAD_TAG_Beam3dThermalAction, theElementTag), theSeries(theSeries),
	ThermalActionType(LOAD_TAG_Beam3dThermalAction)
{
	Loc[0] = locY1; Loc[NumFib_y] = locY2; Loc[NumFib_y+1] = locZ1; Loc[NumFib_y + NumFib_z +1] = locZ2;

	for (int i = 1; i < NumFib_y; i++) {
		Loc[i] = Loc[0] + i * (Loc[NumFib_y] - Loc[0]) / NumFib_y;    //locs through Y
	}
	for (int j = 1; j < NumFib_z; j++) {
		Loc[NumFib_y + 1 + j] = Loc[NumFib_y + 1] + j * (Loc[NumFib_y + NumFib_z + 1] - Loc[NumFib_y + 1]) / NumFib_z;  //locs through Z
	}
	Factors.Zero();
	for (int i = 0; i < ((NumFib_y + 1) * (NumFib_z + 1)); i++) {
		Temp[i] = 0;   //Here the original temp is set as 1, which will be factorized by the value obtained from 
		TempApp[i] = 0;
	}

	indicator = 16;// Using PathTimeSeriesThermal for elemental thermal action with generalised interpolation
}
...
Beam3dThermalAction::getData(int &type, double loadFactor)
{
 ...
 // Added by Anand Kumar 2023
	 ...
	 data.resize((NumFib_y + 1) * (NumFib_z + 1) + NumFib_y + NumFib_z + 2);
	 for (int i = 0; i < ((NumFib_y + 1) * (NumFib_z + 1)); i++) {
		 data(i) = TempApp[i];            // temps across the y-z cross section
	 }
	 for (int i = 0; i < (NumFib_y + NumFib_z + 2); i++)
	 {
		 data((i+ (NumFib_y + 1) * (NumFib_z + 1))) = Loc[i]; //5 locs through Y and then 5 through Z
	 }
 }
 else{
	 data.resize(25);
  for(int i=0; i<5;i++) {
          ...
          data(3*i+10) = TempApp[i+5];       //5 temps through Z
          data(3*i+11)= TempApp[i+10];       //5 temps through Z

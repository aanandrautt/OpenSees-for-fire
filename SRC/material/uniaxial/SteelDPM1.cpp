...
OPS_SteelDPM1()
{
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial SteelDPM1 tag" << endln;
    return 0;
  }
  ...
  if (numData != 3 && numData != 7) {
    opserr << "Invalid #args, want: uniaxialMaterial SteelDPM1 " << iData[0] << " fy? E? b? <a1? a2? a3? a4?>>" << endln;
    return 0;
  }
  ...
  theMaterial = new SteelDPM1(iData[0], dData[0], dData[1], 
				   dData[2], dData[3], dData[4], 
				   dData[5], dData[6]);
  
  
  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type SteelDPM1 Material\n";
    return 0;
  }
...
int SteelDPM1::setTrialStrain(double strain, double FiberTemperature, double strainRate)
{
...
int SteelDPM1::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
   // Reset history variables to last converged state
   TminStrain = CminStrain;
   TmaxStrain = CmaxStrain;
...
void SteelDPM1::determineTrialState (double dStrain)
{
...
double SteelDPM1::getStrain ()
{
   return Tstrain;
}

double SteelDPM1::getStress ()
{
   return Tstress;
}

double SteelDPM1::getTangent ()
{
   return Ttangent; 
}

double 
SteelDPM1::getThermalElongation(void) //***JZ
{
...
//JZ 07/10 /////////////////////////////////////////////////////////////start
double 
SteelDPM1::getElongTangent(double TempT, double &ET, double &Elong, double TempTmax) //PK add to include max temp
{
...
int SteelDPM1::revertToLastCommit ()
{
...
UniaxialMaterial* SteelDPM1::getCopy ()
{
   SteelDPM1* theCopy = new SteelDPM1(this->getTag(), fy, E0, b,
				  a1, a2, a3, a4);
...
int SteelDPM1::sendSelf (int commitTag, Channel& theChannel)
{
   int res = 0;
   ...
   if (res < 0) 
      opserr << "SteelDPM1::sendSelf() - failed to send data\n";

   return res;
}

int SteelDPM1::recvSelf (int commitTag, Channel& theChannel,
                                FEM_ObjectBroker& theBroker)
{
   ...
   if (res < 0) {
      opserr << "SteelDPM1::recvSelf() - failed to receive data\n";
      this->setTag(0);      
   }
   else {
...
void SteelDPM1::Print (OPS_Stream& s, int flag)
{
   s << "SteelDPM1 tag: " << this->getTag() << endln;
   s << "  fy: " << fy << " ";
   s << "  E0: " << E0 << " ";
   s << "  b:  " << b << " ";
   s << "  a1: " << a1 << " ";
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
SteelDPM1::setParameter(const char** argv, int argc, Parameter& param)
{
...
int
SteelDPM1::updateParameter(int parameterID, Information& info)
{
    switch (parameterID) {
    case -1:
        return -1;
...
int
SteelDPM1::activateParameter(int passedParameterID)
{
...
double
SteelDPM1::getStressSensitivity(int gradIndex, bool conditional)
{
...
double
SteelDPM1::getInitialTangentSensitivity(int gradIndex)
{
...
int
SteelDPM1::commitSensitivity(double TstrainSensitivity, int gradIndex, int numGrads)
{
    if (SHVs == 0) {
...
int SteelDPM1::setTrialStrain (double strain, double strainRate)
{
  opserr << "SteelDPM1::setTrialStrain (double strain, double strainRate) - should never be called\n";
  return 0;
}


int 
SteelDPM1::getVariable(const char *variable, Information &info)
{
  if (strcmp(variable,"ThermalElongation") == 0) {
	...
	if (theVector!= 0) {
		...
		opserr<<"null Vector in SteelDPM1"<<endln;
	}
	return 0;
 }
